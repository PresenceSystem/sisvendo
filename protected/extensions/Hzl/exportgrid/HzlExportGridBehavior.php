...
/**
 * Behavior to extend
 * @author Jonathan Urzúa,   Scott Huang
 * @license MIT
 * @version 0.1  ==> Improve to 0.2 at 20140910 by Scott
 */
class HzlExportGridBehavior extends CBehavior {
    ...
    public function toExcel($model = null, $columns = array(), $title = null, $documentDetails = array(), $exportType = 'Excel2007') {
        ...
        if (!$this->owner instanceof CController) {
            Yii::log('HzlExportGridBehavior can only be attached to an instance of CControler', 'error', 'system.base.CBehavior');
            return;
        }
...
//        CVarDumper::dump($arg, 100, true);
//        yii::app()->end();


        $this->owner->widget('ext.Hzl.exportgrid.HzlExportGrid', $arg);
        Yii::app()->end();
    }
